...
topologiaRed.conexiones.forEach( conexion => {
    conexionesPorNodo[conexion.origen]+= 1,
    conexionesPorNodo[conexion.destino]+= 1
});
...
const sugerencias = [];
Object.entries(conexionesPorNodo).forEach(([nodo, cantidadConexiones]) => {
  if (cantidadConexiones > 2) {
    sugerencias.push(
      `El nodo ${nodo} tiene ${cantidadConexiones} conexiones, se recomienda aumentar su ancho de banda.`
    );
  }
});
